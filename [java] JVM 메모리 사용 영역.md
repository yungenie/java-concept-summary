...
- **메서드 영역**  
  \- JVM이 구동될 때 생성되며, 클래스에 대한 정보와 클래스 변수(static variable) Static, final, 문자열 상수(String)가 저장되는 영역이다.  
- **힙 영역**  
  \- 객체와 배열이 생성되는 영역으로 JVM 스택영역에서 생성된 변수, 다른 객체의 필드를 해당 영역의 객체와 필드에 참조하고, GC을 수행합니다.
- **스택 영역**    
...
> \- 해당 영역에 생성된 객체, 배열은 JVM 스택(Stack) 영역에서 생성된 변수나 다른 객체의 필드에서 이 곳에 있는 개체와 배열을 참조한다.  
